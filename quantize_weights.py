...
def quantize_weights(W, method='real', H=1, nb=2, fixed_max=0):
    ...
    elif method == 'radix':  # 1부터 순차적으로 가는 것
        return radix(W, nb)
...
def radix(W, nb):
    ...
    W = map(W, from_min, from_max, -pow(2, nb-1)+1, pow(2, nb-1)-1)
    ...
    # W = tf.floormod(abs(W),nb)*(W/(abs(W)+0.000001))# - (round_through((nb-abs(W))/((nb-abs(W))+0.000001))-1)*W##############
    out = round_through(W)
...
def radix_ReLu(W, nb):
    ...
    # nb=round_through(from_max-from_min) #가중치 정수화만
    # from_max = tf.Print(from_max, [from_max], summarize=1)
    W = map(W, from_min, from_max, 0, pow(2,nb)-1)  # good
...
def quantize(W, nb=16, clip_throughs=False):
    ...
    non_sign_bits = nb - 1
    m = pow(2, non_sign_bits)  # pow(2, 3) -> 2의 3승 -> 8
    # W = tf.Print(W,[W],summarize=20)
    if clip_throughs:
        Wq = clip_through(round_through(W * m), -m, m - 1) / m
        Wq = round_through(Wq) # 반올림
    else:
        Wq = K.clip(round_through(W * m), -m, m - 1) / m + (1 / (m * 2))  # +(1/(m*2))는 가중치들의 값에 0없이 벨런스 맞추기
        Wq = round_through(Wq) # 반올림
...
def quantized_relu(W, nb=16):
    ...
    # K.clip = cliping 해서 0~m-1 범위를 벗어나면 다 잘라냄
    # Wq = K.clip(round_through(W * m), 0, m - 1) / m
    # Wq = tf.Print(Wq, [Wq], summarize=20)

    # nb_bits = nb
    # Wq = K.clip(2. * (round_through(_hard_sigmoid(W) * pow(2, nb_bits)) / pow(2, nb_bits)) - 1., 0, 1 - 1.0 / pow(2, nb_bits - 1))
    return Wq
